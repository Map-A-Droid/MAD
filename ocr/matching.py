...
def fort_image_matching(url_img_name, fort_img_name, zoom, value, raidNo, hash, checkX=False, radius=None, x1=0.30, x2=0.62, y1=0.62, y2=1.23):
    # logger.debug("fort_image_matching: Reading url_img_name '%s'" % str(url_img_name))
    url_img = cv2.imread(url_img_name,3)
    if (url_img is None):
        logger.error('[Crop: ' + str(raidNo) + ' (' + str(hash) +') ] ' + 'fort_image_matching: %s appears to be corrupted' % str(url_img_name))
        return 0.0

    # logger.debug("fort_image_matching: Reading fort_img_name '%s'" % str(fort_img_name))
    fort_img = cv2.imread(fort_img_name,3)
    if (fort_img is None):
        logger.error('[Crop: ' + str(raidNo) + ' (' + str(hash) +') ] ' + 'fort_image_matching: %s appears to be corrupted' % str(fort_img_name))
        return 0.0
    ...
    for scale in np.linspace(npFrom, npValue, matchCount)[::-1]:
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(hash) +') ] ' + 'Filename: ' + str(url_img_name) + ' Matchvalue: ' + str(maxVal))
