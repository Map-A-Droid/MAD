...
class Scanner:
    ...
    def detectRaidTime(self, raidpic, hash, raidNo, radius):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidTime: Reading Raidtimer')
        ...
        logger.debug('[Crop: {} ({})] detectRaidTime: detect raidtimer text: {}', str(raidNo), str(self.uniqueHash), raidtimer)
        # cleanup
        os.remove(emptyRaidTempPath)
        raidFound = len(raidtimer) > 0
        if raidFound:
            if ':' in raidtimer:
                logger.info('[Crop: {} ({})] detectRaidTime: found raidtimer {}', str(raidNo), str(self.uniqueHash), raidtimer)
                hatchTime = self.getHatchTime(raidtimer, raidNo)

                if hatchTime:
                    logger.info('[Crop: {} ({})] ' + 'detectRaidTime: Hatchtime {}', str(raidNo), str(self.uniqueHash), str(hatchTime))
                    ...
                    raidend = hatchTime + (int(self.args.raid_time) * 60)  # (self.timezone * 60 * 60)
                    # raidend = getHatchTime(self, raidtimer) + int(45*60) - (self.timezone*60*60)
                    logger.debug('[Crop: {} ({})] detectRaidTime: Start: {} End: {}', str(raidNo), str(self.uniqueHash), str(raidstart), str(raidend))
                    return (raidFound, True, raidstart, raidend)
                else:
                    logger.info('[Crop: {} ({})] ' + 'detectRaidTime: Could not read Raidtime', str(raidNo), str(self.uniqueHash))
                    return (raidFound, True, False, False)

            else:
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidTime: No correct format for Raidtime found')
                return (raidFound, False, '0', '0')
        else:
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidTime: No RaidTime found')
            return (raidFound, False, False, False)

    def detectRaidEndtimer(self, raidpic, hash, raidNo, radius):
        zero = datetime.datetime.now()
        unixnow = calendar.timegm(zero.timetuple())
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: Reading Raidtimer')
        height, width, channel = raidpic.shape
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: detect raidendtimer text: ' + str(raidtimer))
        ...
        if raidEndFound:
            if ':' in raidtimer:
                ...
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: found raidendtimer {}', raidtimer)
                endTime = self.getEndTime(raidtimer, raidNo)
                if endTime:
                    logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: Endtime {}', str(endTime))
                    if endTime > unixnow + (int(self.args.raid_time) * 60):
                        logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: Endtime not logical')
                        return (raidEndFound, False, False)
                    # raidstart = getHatchTime(self, raidtimer) - self.timezone * (self.timezone*60*60)
                    raidend = endTime  # (self.timezone * 60 * 60)
                    # raidend = getHatchTime(self, raidtimer) + int(45*60) - (self.timezone*60*60)
                    logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: End: {}', str(raidend))
                    return (raidEndFound, True, raidend)
                else:
                    logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: Could not read ReadEndTime')
                    return (raidEndFound, False, False)

            else:
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: No correct format for Raidendtime found')
                return (raidEndFound, False, '0')
        else:
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidEndtimer: No RaidEndTime found')
            return (raidEndFound, False, False)

    def detectRaidBoss(self, raidpic, lvl, hash, raidNo):
        foundmon = None
        monID = None
        logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'Extracting Raidboss')
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidBoss: Scanning Raidboss')
        monHash = None
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidBoss: Monhash: ' + str(monHash))

        if monHash is None:
            for file in glob.glob("ocr/mon_img/_mon_*_" + str(lvl) + ".png"):
                logger.debug("Comparing to {}", str(file))
                find_mon = mon_image_matching(self.args, file, picName, raidNo, hash)
            ...
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectRaidBoss: Found mon in mon_img: ' + str(monID))
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'No Mon found!')
    ...
    def detectLevel(self, raidpic, hash, raidNo, radius):
        ...
        logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'Scanning Level')
        height, width, channel = raidpic.shape
        ...
        if lvl >= 1 and lvl <= 5:
            ...
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectLevel: found level {}', str(lvl))
            return lvl

        logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectLevel: could not find level')
        return None

    def checkDummy(self, raidpic, x1, x2, y1, y2, hash, raidNo, radius):
        foundgym = None
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'checkDummy: Check for dummy Gym Image')

        template = ("ocr/mon_img/dummy_nearby.jpg")

        find_gym = fort_image_matching(raidpic, template, True, 0.9, raidNo, hash, True, radius, x1, x2, y1, y2)
    ...
    def detectGym(self, raidpic, hash, raidNo, captureLat, captureLng, radius, monId=None):
        ...
        logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Scanning Gym')

        # if gymHash is none, we haven't seen the gym yet, otherwise, gymHash == gymId we are looking for
        if monId:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Got Mon-ID for Gym-Detection {}', monId)
            ...
            if str(monId) in data:
                ...
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Found other Crops for Mon {}', monId)
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Cropsizes: x1:{}, x2:{}, y1:{}, y2:{}', str(x1), str(x2), str(y1), str(y2))
        ...
        if gymHash is None:
            ...
            if self.checkDummy(raidpic, x1, x2, y1, y2, hash, raidNo, radius):
                logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Found dummy gym pic')
                self.unknownfound(raidpic, 'unkgym', False, raidNo, hash, False, gymHashvalue, captureLat, captureLng)
                return 'dummy'

            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: No Gym-Hash: found - searching')
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Searching closest gyms')
            if captureLat == '9999':
                distance = 99
            else:
            ...
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Detecting Gym')
            for closegym in closestGymIds:
                ...
                for file in candidates:
                    ...
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Compare Gym-ID - ' + str(closegym[0]) + ' - Match: ' + str(find_gym))
            ...
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Detected Gym-ID: ' + str(gymHash))
            return gymHash

        if gymId:
            logger.info('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'detectGym: Detected Gym - Gym-ID: ' + str(gymId))
    ...
    def unknownfound(self, raidpic, type, zoom, raidNo, hash, captureTime, imageHash=0, lat=0, lng=0):
        ...
        if imageHash:
            ...
            if not glob.glob(existFile):
                ...
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'saveforweb: Write hash file for www: ' + str(type) + "_" + str(lat) + "_" + str(lng) + "_" + str(imageHash) + ".jpg")
            else:
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'saveforweb: Hash file for www already exists: ' + str(type) + "_" + str(lat) + "_" + str(lng) + "_" + str(imageHash) + ".jpg")
        else:
            ...
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'unknownfound: Write unknown file: ' + str(type) + "_" + str(lat) + "_" + str(lng) + "_" + str(time.time()) + ".jpg")
        return True
    ...
    def decodeHashJson(self, hashJson, raidNo):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'Decoding Raid Hash Json')
    ...
    def encodeHashJson(self, gym, lvl, mon, raidNo):
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'Encoding Raid Hash Json')
    ...
    def cropImage(self, image, raidNo, radius):
        ...
        if circles is not None:
            ...
            for (x, y, r) in circles:
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'cropImage: Detect crop coordinates x: ' + str(x) + ' y: ' + str(y) + ' with radius: ' + str(r))
    ...
    def start_detect(self, filenameOfCrop, hash, raidNo, captureTime, captureLat, captureLng, orgFileName, radius):
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Starting detection of crop')
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Original Filename: ' + str(orgFileName))
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Original Lat: ' + str(captureLat))
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Original Lng: ' + str(captureLng))
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Capture Time: ' + str(captureTime))
        if not os.path.isfile(filenameOfCrop):
            logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: File does not exist: {}', str(filenameOfCrop))
            return

        monfound = False
        eggfound = False

        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Starting analysis of ID {}', str(hash))
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Generating Raidhash')
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Got raidtime {}', str(raidtimer))
        ...
        if (not raidtimer[0]):
            ...
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Crop does not show a raid, stopping analysis')
            return False
        ...
        if (not raidstart or not raidend):
            ...
            os.remove(filenameOfCrop)
            os.remove(raidhashPic)
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Crop does not show a valid time, stopping analysis')
            return False

        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Creating Hash overall')
        raidHash = self.imageHashExists(raidhashPic, False, 'raid', raidNo)
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: detectRaidHash: ' + str(raidHash))

        raidlevel = self.detectLevel(img, hash, raidNo, radius)  # we need the raid level to make the possible set of mons smaller
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Determined raidlevel to be {}', str(raidlevel))
        ...
        if raidHash:
            ...
            if lvl == raidlevel and raidlevel is not None:
                ...
                if not mon:
                    ...
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Determined raidlevel to be {}', str(lvl))

                    if lvl is None:
                        logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Could not determine raidlevel. Filename of Crop: {}', filenameOfCrop)
                        os.remove(filenameOfCrop)
                        os.remove(raidhashPic)
                        return True

                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found Raidhash with an egg - fast submit')
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found egg level {} starting at {} and ending at {}. GymID: {}', lvl, raidstart, raidend, gym)
                    ...
                    if raidend[1]:
                        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found Raidhash with an mon and endtime - fast submit')
                        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Submitting mon. ID: {}, gymId: {}', str(mon), str(gym))
                        ...
                        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found Raidhash with an mon - fast submit')
                        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Submitting mon. ID: {}, gymId: {}', str(mon), str(gym))
                ...
                os.remove(filenameOfCrop)
                os.remove(raidhashPic)
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Finished')
                return True
            else:
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: detected Level not Hash Level')
        ...
        if eggfound:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found the crop to contain an egg')
            eggId = eggIdsByLevel[int(raidlevel) - 1]

        if not eggfound:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found the crop to contain a raidboss, lets see what boss it is')
            ...
            if not monFound[0]:
                ...
                logger.warning('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Could not determine mon in crop, aborting and moving crop to unknown')
                ...
                logger.warning('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: could not determine mon, aborting analysis')
                os.remove(raidhashPic)
                os.remove(monUnkPic)
                os.remove(filenameOfCrop)
                return True
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Scanning Gym')
        ...
        if gymId == 'dummy':
            os.remove(raidhashPic)
            os.remove(filenameOfCrop)
            logger.warning('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: determine dummy pic, aborting analysis')
            return True

        if gymId is None:
            # gym unknown...
            logger.warning('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Could not determine gym, aborting analysis')
            os.remove(filenameOfCrop)
            os.remove(raidhashPic)
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: finished')
        ...
        if eggfound:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Found egg level {} starting at {} and ending at {}. GymID: {}', raidlevel, raidstart, raidend, gymId)
            ...
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Adding Raidhash to Database: ' + str(raidHashJson))
            self.imageHash(raidhashPic, raidHashJson, False, 'raid', raidNo)

        else:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Checking for Endtime')
            if not self.db_wrapper.read_raid_endtime(str(gymId), raidNo, unique_hash=self.uniqueHash):
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: No existing Egg found')
                raidend = self.detectRaidEndtimer(img, hash, raidNo, radius)
                if raidend[1]:
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Submitting mon without egg. ID: {}, gymId: {}', str(monFound[0]), str(gymId))
                    ...
                    logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Submitting mon without raidend. ID: {}, gymId: {}', str(monFound[0]), str(gymId))
                ...
                logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'start_detect: Submitting mon with previously reported endtime. ID: {}, gymId: {}', str(monFound[0]), str(gymId))
                submitStatus = self.db_wrapper.submit_raid(str(gymId), monFound[0], raidlevel, None, None, 'MON',
                                                           raidNo, captureTime, unique_hash=self.uniqueHash)
    ...
    def dhash(self, image, raidNo, hash_size=8):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'ImageHash: Generated Image Hash: ' + str(hashValue))
        return hashValue
    ...
    def imageHashExists(self, image, zoom, type, raidNo, x1=0.30, x2=0.62, y1=0.62, y2=1.23, radius=0, hashSize=8):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'imageHashExists: Hash found: {}', existHash[1])
        return existHash[1]
    ...
    def imageHash(self, image, id, zoom, type, raidNo, x1=0.30, x2=0.62, y1=0.62, y2=1.23, radius=0, hashSize=8):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' +  'imageHash: ' + str(imageHash))
        ...
        existHash = self.db_wrapper.check_for_hash(str(imageHash), str(type), raidNo, distance=distance,
                                                   unique_hash=self.uniqueHash)
        if existHash[0]:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'imageHash: Hash already in Database: ' + str(existHash[2]))
            self.db_wrapper.insert_hash(str(existHash[2]), str(type), str(id), raidNo, unique_hash=self.uniqueHash)
            return str(existHash[2])
        else:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'imageHash: Adding Hash to Database: ' + str(imageHash))
            self.db_wrapper.insert_hash(str(imageHash), str(type), str(id), raidNo, unique_hash=self.uniqueHash)
    ...
    def getImageHash(self, image, zoom, raidNo, type, x1=0.30, x2=0.62, y1=0.62, y2=1.23, radius=0, hashSize=8):
        ...
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getImageHash: ' + str(imageHash))
        os.remove(tempHash)
        ...
        existHash = self.db_wrapper.check_for_hash(str(imageHash), str(type), raidNo, distance=distance,
                                                   unique_hash=self.uniqueHash)
        if existHash[0]:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getImageHash: Hash already in Database: ' + str(existHash[2]))
            return str(existHash[2])
        else:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getImageHash: Hash not in Database: ' + str(imageHash))
            return str(imageHash)
    ...
    def getHatchTime(self, data, raidNo):
        ...
        if hour_min is None:
            logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ]' + 'getHatchTime: Could not locate a HH:MM')
            return False
        else:
        ...
        if am_found:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getHatchTime: Found AM')
            return int(unix_zero)+int(hour_min[0])*3600+int(hour_min[1])*60
        elif pm_found:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getHatchTime: Found PM')
            if hour_min[0] == '12':
                return int(unix_zero)+int(hour_min[0])*3600+int(hour_min[1])*60
            else:
                return int(unix_zero)+(int(hour_min[0])+12)*3600+int(hour_min[1])*60
        else:
            logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getHatchTime: Found EU Time')
            return int(unix_zero)+int(hour_min[0])*3600+int(hour_min[1])*60
    ...
    def getEndTime(self, data, raidNo):
        ...
        unix_zero = calendar.timegm(zero.timetuple())
        hour_min_divider = data.find(':')
        logger.debug('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getHatchTime: :Count: ' + str(data.count(':')))
        if data.count(':') < 2:
            logger.error('[Crop: ' + str(raidNo) + ' (' + str(self.uniqueHash) + ') ] ' + 'getHatchTime: Detect wrong Endtimer of Raid')
            return False
