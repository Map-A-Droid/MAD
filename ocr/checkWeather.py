...
def weather_image_matching(weather_icon_name, screenshot_name):
    ...
    if weather_icon is None:
        logger.error('weather_image_matching: %s appears to be corrupted' % str(url_img_name))
        return 0

    screenshot_img = cv2.imread(screenshot_name, 3)

    if screenshot_img is None:
        logger.error('weather_image_matching: %s appears to be corrupted' % str(screenshot_name))
        return 0
...
def checkWeather(raidpic):
    ...
    if foundweather[0] > 0:
        ...
        logger.info('The weather on the screenshot could be identified  (%s)' % str(weatherName[0].replace('_', ' ')))
