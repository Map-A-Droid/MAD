...
class WorkerOCR(WorkerBase):
    ...
    def _post_move_location_routine(self, timestamp):
        ...
        if not self._takeScreenshot():
            ...
            logger.debug("Worker: couldn't take screenshot before radscreen check, lock released")
            return

        logger.debug("Worker: Got screenshot")
        # curTime = time.time()
        logger.info("main: Checking raidcount and copying raidscreen if raids present")
        ...
        if count_of_raids == -1:
            ...
            logger.warning("main: Count present but no raid shown, reopening raidTab")
            self._reopenRaidTab()
            logger.debug("Done reopening raidtab")
            if not self._takeScreenshot():
                self._work_mutex.release()
                logger.debug("Worker: couldn't take screenshot after opening raidtab, lock released")
                return
            count_of_raids = self._pogoWindowManager.readRaidCircles(os.path.join(
                self._applicationArgs.temp_path, 'screenshot%s.png' % str(self._id)), self._id, self._communicator)
        #    elif countOfRaids == 0:
    ...
    def _speed_weather_check_thread(self):
        while not self._stop_worker_event.is_set():
            ...
            try:
                ...
                logger.debug("checkSpeedWeatherWarningThread: Checking if pogo is running...")
                if not self._communicator.isPogoTopmost():
                    logger.warning("checkSpeedWeatherWarningThread: Starting Pogo")
                    self._restart_pogo()

                reached_raidscreen = self._getToRaidscreen(10, True)

                if reached_raidscreen:
                    logger.debug("checkSpeedWeatherWarningThread: checkSpeedWeatherWarningThread: reached raidscreen...")
