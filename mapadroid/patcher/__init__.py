...
class MADPatcher(object):
    ...
    def __get_installed_version(self):
        try:
            ...
            if self._installed_ver:
                ...
                logger.success(
                    "Moved internal MAD version to database as version {}", self._installed_ver,
                )
        except Exception:
            logger.opt(exception=True).critical(
                "Unknown exception occurred during getting the MAD DB version." "  Exiting"
            )
    ...
    def __install_schema(self):
        try:
            ...
            logger.success(
                "Successfully installed MAD version {} to the database", self._installed_ver,
            )
            self.__add_default_event()
            reload_instance_id(self.data_manager)
        except Exception:
            logger.opt(exception=True).critical(
                "Unable to install default MAD schema.  Please install the schema from " "scripts/SQL/rocketmap.sql"
            )
            sys.exit(1)
    ...
    def __update_mad(self):
        if self._madver < self._installed_ver:
            ...
            logger.warning(
                "Performing updates from version {} to {} now", self._installed_ver, self._madver,
            )
                ...
                if column["Key"] != "PRI":
                    ...
                    try:
                        ...
                        if err.errno == 1062:
                            ...
                            logger.success(
                                "Successfully de-duplicated versions table and set each key to the "
