...
class Patch(PatchBase):
    ...
    def _execute(self):
        ...
        if not self._schema_updater.check_column_exists("trs_quest", "quest_reward"):
            alter_query = "ALTER TABLE trs_quest " "ADD quest_reward VARCHAR(500) NULL AFTER quest_condition"
            try:
        ...
        if not self._schema_updater.check_column_exists("trs_quest", "quest_task"):
            alter_query = "ALTER TABLE trs_quest " "ADD quest_task VARCHAR(150) NULL AFTER quest_reward"
            try:
                self._db.execute(alter_query, commit=True)
            except Exception as e:
                self._logger.info("Unexpected error: {}", e)
        ...
        alter_query = "ALTER TABLE raid " "ADD form smallint(6) DEFAULT NULL"
        column_exist = self._schema_updater.check_column_exists("raid", "form")
        if not column_exist:
            try:
                self._db.execute(alter_query, commit=True)
            except Exception as e:
