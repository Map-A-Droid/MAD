    ...
    def _execute(self):
        try:
            ...
            for section in update_order:
                ...
                self._logger.info(
                    "Found a {} with an ID of 0.  Setting ID to {}", section, cache[section],
                )
                # Update the trs_status table to reflect the latest
                if section == "areas":
                    updated = {"routemanager": cache[section]}
                    where = {"routemanager": 0}
            ...
            for section in update_order:
                ...
                for elem_id, elem in config_file[section]["entries"].items():
                    ...
                    if not exists:
                        continue
                    self._logger.info(
                        "{} {} already exists and a new ID will be generated", dm_section, elem_id,
                    )
            ...
            for section in update_order:
                ...
                for key, elem in copy.deepcopy(config_file[section]["entries"]).items():
                    ...
                    if section == "areas":
                        mode = elem["mode"]
                        del elem["mode"]
                        resource = mapadroid.data_manager.modules.MAPPINGS["area"](self._data_manager, mode=mode)
                        ...
                        resource = mapadroid.data_manager.modules.MAPPINGS[dm_section](self._data_manager)
                    ...
                    if dm_section == "area":
                        try:
                            ...
                            self._logger.info(
                                "Updating monivlist from {} to {}", key, elem["settings"]["mon_ids_iv"],
                            )
                        except KeyError:
                            pass
                        ...
                        try:
                            ...
                            self._logger.info(
                                "Updating device pool from {} to {}", pool_id, elem["pool"],
                            )
                        except KeyError:
                            pass
                        try:
                            ...
                            self._logger.info(
                                "Updating device walker from {} to {}", walker_id, elem["walker"],
                            )
                        except KeyError:
                            pass
                        ...
                        for walkerarea_id in elem["setup"]:
                            try:
                                ...
                                self._logger.info(
                                    "Updating walker-walkerarea from {} to {}", walkerarea_id, new_list[-1],
                                )
                            except KeyError:
                        ...
                        try:
                            ...
                            self._logger.info(
                                "Updating walkerarea from {} to {}", area_id, elem["walkerarea"],
                            )
                        except KeyError:
                            pass
                    save_new_id = False
