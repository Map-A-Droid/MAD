...
class Patch(PatchBase):
    ...
    def _execute(self):
        ...
        sql = "ALTER TABLE `trs_spawn` " "CHANGE `spawnpoint` `spawnpoint` bigint(20) unsigned NOT NULL"
        try:
            self._db.execute(sql, commit=True, suppress_log=True)
        except Exception:
            pass
        ...
        sql = "ALTER TABLE `trs_spawn` " "ADD KEY `event_lat_long` (`eventid`, `latitude`, `longitude`)"
        try:
            self._db.execute(sql, commit=True, suppress_log=True)
        except Exception:
