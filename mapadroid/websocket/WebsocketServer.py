...
from mapadroid.utils import (WebsocketWorkerRemovedException,
    ...
    async def __register(self, websocket_client_connection):
        ...
        try:
            ...
            if walker_routemanager_mode is None:
                ...
                worker = WorkerMITM(self.args, origin, last_known_state, self,
                ...
                worker = WorkerQuests(self.args, origin, last_known_state, self,
                                      routemanager_name=walker_area_name,
                                      mitm_mapper=self.__mitm_mapper, mapping_manager=self.__mapping_manager,
                                      db_wrapper=self.__db_wrapper,
            ...
            logger.error(
                "Other unhandled exception during register: {}\n{}, {}".format(e.with_traceback(None),
                                                                               exc_value, str(e)))
            await websocket_client_connection.close()
        finally:
            async with self.__users_mutex:
            ...
            with self.__loop_mutex:
                future = asyncio.run_coroutine_threadsafe(
                    self.__send_and_wait_internal(id, worker_instance, message, timeout,
                                                  byte_command=byte_command), self.__loop)
