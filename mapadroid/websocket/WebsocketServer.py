...
from mapadroid.utils.logging import InterceptHandler, LoggerEnums, get_logger, get_origin_logger
...
class WebsocketServer(object):
    def __init__(
        ...
        self.__worker_factory: WorkerFactory = WorkerFactory(
            self.__args,
            self.__mapping_manager,
            self.__mitm_mapper,
            self.__db_wrapper,
            self.__pogo_window_manager,
            event,
        )
        ...
        self.__internal_worker_join_thread: Thread = Thread(name="system", target=self.__internal_worker_join)
        ...
        async with self.__current_users_mutex:
            for worker_entry in self.__current_users.values():
                ...
                await self.__close_websocket_client_connection(
                    worker_entry.origin, worker_entry.websocket_client_connection
                )
    ...
    def stop_server(self) -> None:
        ...
        future = asyncio.run_coroutine_threadsafe(self.__close_all_connections_and_signal_stop(), self.__loop)
        future.result()
    ...
    def __internal_worker_join(self):
        while not self.__stop_server.is_set() or (
            self.__stop_server.is_set() and not self.__worker_shutdown_queue.empty()
        ):
            try:
            ...
            if next_item is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "Caught runtime error trying to join thread, the thread likely did not start at all."
                        " Exact message: {}",
                        e,
                    )
    ...
    async def __connection_handler(
        self, websocket_client_connection: websockets.WebSocketClientProtocol, path: str
    ) -> None:
        ...
        try:
            ...
            async with self.__users_connecting_mutex:
                self.__users_connecting.remove(origin)
            receiver_task = asyncio.ensure_future(self.__client_message_receiver(origin, entry))
            await receiver_task
    ...
    async def __add_worker_and_thread_to_entry(self, entry, origin, use_configmode: bool = None) -> bool:
        communicator: AbstractCommunicator = Communicator(entry, origin, None, self.__args.websocket_command_timeout)
        use_configmode: bool = use_configmode if use_configmode is not None else self.__enable_configmode
        worker: Optional[AbstractWorker] = await self.__worker_factory.get_worker_using_settings(
            origin, use_configmode, communicator=communicator
        )
        if worker is None:
            return False
    ...
    async def __authenticate_connection(
        ...
        """
        try:
            origin = str(websocket_client_connection.request_headers.get_all("Origin")[0])
        except IndexError:
            logger.warning(
                "Client from {} tried to connect without Origin header", websocket_client_connection.remote_address,
            )
            return (None, False)
        origin_logger = get_origin_logger(logger, origin=origin)
        ...
        if self.__mapping_manager is None:
            origin_logger.warning(
                "No configuration has been defined.  Please define in MADmin and click " "'APPLY SETTINGS'"
            )
            (origin, False)
        ...
        if valid_auths:
            try:
                auth_base64 = str(websocket_client_connection.request_headers.get_all("Authorization")[0])
            except IndexError:
    ...
    @staticmethod
    async def __close_websocket_client_connection(
        origin_of_worker: str, websocket_client_connection: websockets.WebSocketClientProtocol,
    ) -> None:
    ...
    def get_reg_origins(self) -> List[str]:
        future = asyncio.run_coroutine_threadsafe(self.get_connected_origins(), self.__loop)
        return future.result()

    def get_origin_communicator(self, origin: str) -> Optional[AbstractCommunicator]:
        # TODO: this should probably lock?
        entry: Optional[WebsocketConnectedClientEntry] = self.__current_users.get(origin, None)
        return entry.worker_instance.communicator if entry is not None and entry.worker_instance is not None else False

    def set_geofix_sleeptime_worker(self, origin: str, sleeptime: int) -> bool:
        entry: Optional[WebsocketConnectedClientEntry] = self.__current_users.get(origin, None)
        return (
            entry.worker_instance.set_geofix_sleeptime(sleeptime)
    ...
    async def __close_and_signal_stop(self, origin: str) -> None:
        ...
        async with self.__current_users_mutex:
            ...
            if entry is not None:
                entry.worker_instance.stop_worker()
                await self.__close_websocket_client_connection(entry.origin, entry.websocket_client_connection)
    ...
    def force_disconnect(self, origin) -> None:
        future = asyncio.run_coroutine_threadsafe(self.__close_and_signal_stop(origin), self.__loop)
        future.result()
