...
class WordToScreenMatching(object):
    def __init__(
        self, communicator, pogo_win_manager, origin, resocalc, mapping_mananger: MappingManager, args,
    ):
        self.origin = origin
    ...
    def get_next_account(self):
        ...
        if self._logintype == LoginType.ptc:
            ...
            self._logger.info(
                "Using GGL Account: {}", self.censor_account(self._GGL_accounts[self._accountindex - 1].username),
            )
    ...
    def __evaluate_topmost_app(self, topmost_app: str) -> Tuple[ScreenType, dict, int]:
        ...
        if "AccountPickerActivity" in topmost_app or "SignInActivity" in topmost_app:
            ...
            if not self._take_screenshot(
                delay_before=self.get_devicesettings_value("post_screenshot_delay", 1), delay_after=2,
            ):
    ...
    def _click_center_button(self, diff, global_dict, i) -> None:
        (x, y, w, h) = (
            ...
            global_dict["top"][i],
            global_dict["width"][i],
            global_dict["height"][i],
        )
        self._logger.debug("Diff: {}", diff)
        click_x, click_y = (x + w / 2) / diff, (y + h / 2) / diff
        self._communicator.click(click_x, click_y)

    def __handle_screentype(
        self, screentype: ScreenType, global_dict: Optional[dict] = None, diff: int = -1
    ) -> ScreenType:
    ...
    def __check_pogo_screen_ban_or_loading(self, screentype) -> ScreenType:
        ...
        if backgroundcolor is not None and (
            backgroundcolor[0] == 0 and backgroundcolor[1] == 0 and backgroundcolor[2] == 0
        ):
            # Background is black - Loading ...
            screentype = ScreenType.BLACK
        elif backgroundcolor is not None and (
            backgroundcolor[0] == 16 and backgroundcolor[1] == 24 and backgroundcolor[2] == 33
        ):
    ...
    def __handle_returning_player_or_wrong_credentials(self) -> None:
        ...
        self._pogoWindowManager.look_for_button(
            self.origin, self.get_screenshot_path(), 2.20, 3.01, self._communicator, upper=True,
        )
        time.sleep(2)
    ...
    def get_devicesettings_value(self, key: str, default_value: object = None):
        ...
        try:
            ...
            self._logger.warning(
                "Failed fetching devicemappings in worker with description: {}. Stopping worker", e,
            )
            return None
