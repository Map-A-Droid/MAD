...
from mapadroid.tests.test_utils import GetStorage, ResourceCreator, get_connection_api, get_connection_mitm
...
pd_conf = {"post_destination": pd_host}
rgc_host = "ws://192.168.1.69:8080"
rgc_conf = {"websocket_uri": rgc_host}


def create_valid_configs(
    test_obj, api_creator, auth: bool = True, pd_conf: dict = pd_conf, rgc_conf: dict = rgc_conf,
):
    conf_pd = copy.deepcopy(pd_conf)
    conf_rgc = copy.deepcopy(rgc_conf)
    if auth:
        payload = {"username": email_base, "password": pwd_base}
...
class MITMAutoConf(unittest.TestCase):
    ...
    def test_no_configured_endpoints(self):
        ...
        try:
            ...
            with GetStorage(self.api) as storage:
                ...
                expected_issues = {
                    "X-Critical": [AutoConfIssues.pd_not_configured.value, AutoConfIssues.rgc_not_configured.value,],
                    "X-Warnings": [AutoConfIssues.no_ggl_login.value, AutoConfIssues.auth_not_configured.value,],
                }
    ...
    def test_duplicate_mac_update(self):
        ...
        headers = {"Origin": dev_info["origin"]}
