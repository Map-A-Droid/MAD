...
class APIArea(api_base.APITestBase):
    ...
    def test_get_modes(self):
        ...
        resource_resp = (
            "Please specify a mode for resource information Valid modes: idle,iv_mitm,mon_mitm,pokestops," "raids_mitm"
        )
    ...
    def test_invalid_post_mode(self):
        payload = {}
        err = (
            "Please specify a mode for resource information.  Valid modes: idle,iv_mitm,mon_mitm,pokestops,"
            "raids_mitm"
        )
        errors = {"error": err}
        super().invalid_post(payload, errors, error_code=400)
        headers = {"X-Mode": "fake-mode"}
        errors = {
    ...
    def test_invalid_post(self):
        payload = {
            ...
            "routecalc": "test_calc",
            "username": "ya",
            "settings": {"starve_route": False},
        }
        headers = {"X-Mode": "raids_mitm"}
        errors = {
            "unknown": ["username"],
            "invalid": [["geofence_included", "Integer (1,2,3)"], ["routecalc", "Integer (1,2,3)"],],
            "invalid_uri": [
    ...
    def test_issue_495(self):
        ...
        patch = {"settings": {"delay_after_prio_event": None}}
    ...
    def test_invalid_geofence_settings(self):
        area_obj = super().create_valid_resource("area")
        patch = {"geofence_excluded": area_obj["resources"]["geofence_included"]["uri"]}
    ...
    def test_invalid_initlevel_settings(self):
        ...
        headers = {"X-Mode": "pokestops"}
        area_obj, resp = self.creator.create_valid_resource("area", payload=payload, headers=headers)
        patch = {"init": True, "level": True}
        response = self.api.patch(area_obj["uri"], json=patch)
        expected = {
            "invalid": [
                [
                    "init",
                    "Cannot have init and level set to True at the same time. "
