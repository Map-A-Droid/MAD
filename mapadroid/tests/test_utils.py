...
from mapadroid.mad_apk import APKArch, APKType, PackageImporter, get_storage_obj
...
class GetStorage(object):
    ...
    def upload_pd(self, reload: bool = True):
        upload_package(
            self.storage_elem, version="0.1", apk_type=APKType.pd, apk_arch=APKArch.noarch,
        )
        if reload:
            self.api.get("/api/mad_apk/reload")
    ...
    def upload_rgc(self, reload: bool = True):
        upload_package(
            self.storage_elem, version="0.1", apk_type=APKType.rgc, apk_arch=APKArch.noarch,
        )
        if reload:
            self.api.get("/api/mad_apk/reload")
...
def upload_package(
    storage_elem, version: str = None, apk_type: APKType = APKType.rgc, apk_arch: APKArch = APKArch.noarch,
) -> NoReturn:
    data = get_rgc_bytes()
    if version is None:
