...
class GeofenceHelper:
    def __init__(self, include_geofence, exclude_geofence, fence_name=None):
        ...
        if include_geofence or exclude_geofence:
            ...
            self.excluded_areas = self.parse_geofences_file(exclude_geofence, excluded=True, fence_fallback=fence_name)
            logger.debug2(
                "Loaded {} geofenced and {} excluded areas.", len(self.geofenced_areas), len(self.excluded_areas),
            )
        ...
        if geo_resource:
            ...
            for line in geo_resource["fence_data"]:
                ...
                if len(line) == 0:  # Empty line.
                    ...
                    geofences.append({"excluded": excluded, "name": name, "polygon": []})
                    ...
                    if first_line:
                        ...
                        geofences.append({"excluded": excluded, "name": fencename, "polygon": []})
    ...
    def is_point_in_polygon_custom(point, polygon):
        ...
        if (point["lat"] > max_lat) or (point["lat"] < min_lat) or (point["lon"] > max_lon) or (point["lon"] < min_lon):
            return False
        ...
        for poly_point in range(1, poly_sides + 1):
            lat2, lon2 = (
                polygon[poly_point % poly_sides]["lat"],
                polygon[poly_point % poly_sides]["lon"],
            )
            if min(lon1, lon2) < point["lon"] <= max(lon1, lon2) and point["lat"] <= max(lat1, lat2):
                if lon1 != lon2:
