...
class PlayerStats(AbstractStatsHolder):
    def __init__(self, origin: str, db_wrapper: DbWrapper, application_args):
    ...
    async def stats_collect_wild_mon(self, encounter_id: int, time_scanned: datetime):
        self._wild_mon_stats_holder.add(encounter_id, time_scanned, is_shiny=False)
        self._stats_detect_holder.add_mon(time_scanned)

    async def stats_collect_mon_iv(self, encounter_id: int, time_scanned: datetime, is_shiny: bool):
    ...
    async def stats_collect_quest(self, time_scanned: datetime):
        self._stats_detect_holder.add_quest(time_scanned)

    async def stats_collect_raid(self, time_scanned: datetime):
        self._stats_detect_holder.add_raid(time_scanned)

    async def stats_collect_location_data(self, location: Location, success: bool, fix_timestamp: int,
