...
from mapadroid.mad_apk import APKType, lookup_package_info, parse_frontend, stream_package, supported_pogo_version
from mapadroid.mitm_receiver.MitmMapper import MitmMapper
from mapadroid.utils import MappingManager
from mapadroid.utils.authHelper import check_auth
from mapadroid.utils.autoconfig import PDConfig, RGCConfig, origin_generator
from mapadroid.utils.collections import Location
from mapadroid.utils.logging import LoggerEnums, LogLevelChanger, get_logger, get_origin_logger
    ...
    def __call__(self, *args, **kwargs):
        ...
        if request.url_rule is not None and str(request.url_rule) == "/status/":
            ...
            if origin is None:
                ...
                self.mapping_manager.get_all_devicemappings().keys() is not None
                and origin not in self.mapping_manager.get_all_devicemappings().keys()
            ):
                ...
                if auth is None or not check_auth(
                    origin_logger, auth, self.application_args, self.mapping_manager.get_auths(),
                ):
                    origin_logger.warning("Unauthorized attempt to POST from {}", request.remote_addr)
                    self.response = Response(status=403, headers={})
                    abort = True
...
class MITMReceiver(Process):
    def __init__(
        ...
        name=None,
        enable_configmode: Optional[bool] = False,
    ):
        ...
        if not enable_configmode:
            self.add_endpoint(
                endpoint="/", endpoint_name="receive_protos", handler=self.proto_endpoint, methods_passed=["POST"],
            )
            self.add_endpoint(
                endpoint="/get_latest_mitm/",
                endpoint_name="get_latest_mitm/",
                handler=self.get_latest,
                methods_passed=["GET"],
            )
            self.add_endpoint(
                endpoint="/status/", endpoint_name="status/", handler=self.status, methods_passed=["GET"],
            )
    ...
    def __handle_proto_data_dict(self, origin: str, data: dict) -> None:
        ...
        if (
            location_of_data.lat > 90
            or location_of_data.lat < -90
            or location_of_data.lng > 180
            or location_of_data.lng < -180
        ):
    ...
    def get_latest(self, origin, data):
        ...
        unquest_stops = self.__mitm_mapper.request_latest(origin, "unquest_stops")
    ...
    def autoconfig_get_config(self, *args, **kwargs) -> Response:
        ...
        try:
            ...
            if operation in ["pd", "rgc"]:
                ...
                sql = (
                    "SELECT ag.`username`, ag.`password`\n"
    ...
    def autoconfig_status(self, *args, **kwargs) -> Response:
        session_id: Optional[int] = kwargs.get("session_id", None)
        update_data = {"ip": get_actual_ip(request)}
        where = {"session_id": session_id, "instance_id": self._db_wrapper.instance_id}
        self._db_wrapper.autoexec_update("autoconfig_registration", update_data, where_keyvals=where)
        return Response(status=200)
