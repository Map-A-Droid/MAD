...
class PlayerStats(object):
        ...
        if len(stats) > 0:
            for data_inventory in stats:
                ...
                if int(player_level) > 0:
                    ...
                    with open(
                        os.path.join(self.__application_args.file_path, str(self._id) + ".stats"), "w",
                    ) as outfile:
                        json.dump(data, outfile, indent=4, sort_keys=True)

    def open_player_stats(self):
        statsfile = Path(os.path.join(self.__application_args.file_path, str(self._id) + ".stats"))
        try:
    ...
    def compare_hour(self, timestamp):
        if datetime.datetime.fromtimestamp(int(time.time())).strftime("%H") != datetime.datetime.fromtimestamp(
            int(timestamp)
        ).strftime("%H"):
            return True
        return False
    ...
    def stats_collector(self):
        ...
        with self.__mapping_mutex:
            if not self._stats_collector_start:
                time_diff = time.time() - self._last_processed_timestamp
                if time_diff >= self.__application_args.game_stats_save_time or self.compare_hour(
                    self._last_processed_timestamp
                ):
                    self._last_processed_timestamp = time.time()

                    self.__mitm_mapper_parent.add_stats_to_process(
                        self._id, self.__stats_collected.copy(), self._last_processed_timestamp,
                    )
    ...
    def stats_collect_location_data(
        self, location, datarec, start_timestamp, positiontype, rec_timestamp, walker, transporttype,
    ):
        if not self._generate_stats:
            return
        with self.__mapping_mutex:
    ...
    def stats_location_raw_parser(client_id: str, data, period):
        ...
        origin_logger.debug4(
            "Submit raw location stats - Period: {} - Count: {}", period, len(data_location_raw),
        )
        ...
        if 106 in data:
            if "mon" in data[106]:
                for mon_id in data[106]["mon"]:
                    ...
                    data_location_raw.append(
                        (str(client_id), str(type_id), "mon", str(type_count), 0, str(int(period)),)
                    )
            ...
            if "raid" in data[106]:
                for gym_id in data[106]["raid"]:
                    ...
                    data_location_raw.append(
                        (str(client_id), str(type_id), "raid", str(type_count), 0, str(int(period)),)
                    )
            ...
            if "quest" in data[106]:
                for stop_id in data[106]["quest"]:
                    ...
                    data_location_raw.append(
                        (str(client_id), str(type_id), "quest", str(type_count), 0, str(int(period)),)
                    )
        ...
        if 102 in data:
            if "mon_iv" in data[102]:
                for mon_id in data[102]["mon_iv"]:
                    ...
                    data_location_raw.append(
                        (str(client_id), str(type_id), "mon_iv", str(type_count), shiny, str(int(period)),)
                    )

        origin_logger.debug(
            "Submit raw detection stats for Period: {} - Count: {}", period, len(data_location_raw),
        )
        return data_location_raw
