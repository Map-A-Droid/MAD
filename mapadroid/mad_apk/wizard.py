...
from .utils import get_apk_info, is_newer_version, lookup_arch_enum, lookup_package_info, supported_pogo_version
...
class APKWizard(object):
    ...
    def download_pogo(self, architecture: APKArch) -> NoReturn:
        ...
        if latest_pogo_info is None:
            ...
            if current_version is None or is_newer_version(latest_version, current_version):
                where = {"usage": APKType.pogo.value, "arch": architecture.value}
                try:
    ...
    def find_latest_pogo(self, architecture: APKArch) -> Optional[str]:
        ...
        try:
            ...
            if (
                ...
                or is_newer_version(latest_supported["version"], current_version)
    ...
    def get_latest_version(self) -> Dict[APKArch, str]:
        ...
        for ver_identifier, version_code in gh_resp.json().items():
            ...
            if versions[named_arch] is None:
                versions[named_arch] = {"versionCode": version_code, "version": version}
    ...
    def get_version_code(self, latest_supported: Dict[APKArch, str], arch: APKArch) -> Optional[int]:
        ...
        pogo_vc_sess.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"})
    ...
    def set_last_searched(
        self, package: APKType, architecture: APKArch, version: str = None, url: str = None,
    ) -> NoReturn:
