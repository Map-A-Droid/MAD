...
def generator_from_db(dbc, package: APKType, architecture: APKArch) -> Generator:
    ...
    filestore_id = dbc.autofetch_value(filestore_id_sql, args=(package.value, architecture.value,),)
...
def lookup_package_info(
    storage_obj: AbstractAPKStorage, package: APKType, architecture: APKArch = None
) -> Tuple[Union[MADPackage, MADPackages], int]:
...
def stream_package(db, storage_obj, package: APKType, architecture: APKArch) -> Response:
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename={}".format(package_info.filename)},
    )
