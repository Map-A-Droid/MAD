...
class APKStorageDatabase(AbstractAPKStorage):
    ...
    def delete_file(self, package: APKType, architecture: APKArch) -> bool:
        ...
        filestore_id: Optional[int] = self.dbc.autofetch_value(
            filestore_id_sql, args=(package.value, architecture.value,),
        )
        if filestore_id:
    ...
    def get_current_package_info(self, package: APKType) -> Optional[MADPackages]:
        ...
        data = MADPackages()
        sql = (
            "SELECT ma.`version`, ma.`arch`, fm.`filename`, fm.`size`, fm.`mimetype`\n"
            "FROM `mad_apks` ma\n"
    ...
    def save_file(
        ...
        try:
            ...
            for chunked_data in [
                data.getbuffer()[i * chunk_size : (i + 1) * chunk_size]
                for i in range((len(data.getbuffer()) + chunk_size - 1) // chunk_size)
            ]:
                insert_data = {
                    "filestore_id": new_id,
