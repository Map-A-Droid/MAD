...
class RouteCalc(Resource):
    ...
    def save(
        ...
        return super().save(
            core_data=core_data, force_insert=force_insert, ignore_issues=ignore_issues, literals=literals,
        )
    ...
    def validate_custom(self) -> Dict[str, List[Tuple[str, str]]]:
        ...
        for line, row in enumerate(self["routefile"]):
            ...
            if len(row_split) != 2:
                if not invalid_data:
                    issues = {"invalid": [("routefile", "Must be one coord set per line (float,float)",)]}
    ...
    def get_json_route(
        ...
        if not in_memory and (
            ...
            for line in self._data["fields"]["routefile"]:
                ...
                export_data.append({"lat": float(line_split[0].strip()), "lng": float(line_split[1].strip())})
            return export_data
        ...
        if len(coords) > 0 and max_radius and max_coords_within_radius:
            ...
            logger.debug(
                "Coords summed up: {}, that's just {} coords", less_coords, len(less_coords),
            )
        ...
        if len(less_coords) < 3:
            ...
            export_data = []
            for i in range(len(less_coords)):
                export_data.append({"lat": less_coords[i][0].item(), "lng": less_coords[i][1].item()})
        else:
            ...
            for i in range(len(sol_best)):
                export_data.append(
                    {"lat": less_coords[int(sol_best[i])][0].item(), "lng": less_coords[int(sol_best[i])][1].item()}
                )
        if not in_memory:
            calc_coords = []
    ...
    def get_less_coords(
        ...
        clustering_helper = ClusteringHelper(
            ...
            max_count_per_circle=max_coords_within_radius,
