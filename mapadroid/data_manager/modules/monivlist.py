...
class MonIVList(Resource):
    ...
    search_field = "name"
    translations = {"monlist": "name"}
    configuration = {
        "fields": {
            "monlist": {
                "settings": {"type": "text", "require": True, "description": "Name of global Monlist", "expected": str}
            },
            "mon_ids_iv": {
                "settings": {
                    "type": "text",
    ...
    def save(self, force_insert: Optional[bool] = False, ignore_issues: Optional[List[str]] = None,) -> int:
        ...
        for ind, mon in enumerate(self._data["fields"]["mon_ids_iv"]):
            mon_data = {"monlist_id": self.identifier, "mon_id": mon, "mon_order": ind}
            try:
                self._dbc.autoexec_insert("settings_monivlist_to_mon", mon_data)
            except mysql.connector.Error:
                logger.info(
                    "Duplicate pokemon {} detected in list {}", mon, self.identifier,
                )
