...
class Area(Resource):
    ...
    primary_key = "area_id"
    search_field = "name"
    translations = {"mon_ids_iv": "monlist_id"}
    ...
    def _load(self) -> None:
        ...
        for field, db_value in mode_data.items():
            ...
            elif "settings" in self.configuration and field in self.configuration["settings"] and db_value is not None:
    ...
    def save(self, force_insert: Optional[bool] = False, ignore_issues: Optional[List[str]] = None,) -> int:
        ...
        core_data = {"name": self._data["fields"]["name"], "mode": self.area_type}
        try:
    ...
    def validate_custom(self) -> Dict[str, List[Tuple[str, str]]]:
        ...
        try:
            if self._data["fields"]["geofence_included"] == self._data["fields"]["geofence_excluded"]:
                issues = {"invalid": [("geofence_excluded", "Cannot be the same as geofence_included")]}
        except KeyError:
            pass
        try:
