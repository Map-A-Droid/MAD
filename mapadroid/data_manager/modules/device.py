...
pogoauth_fields = {"ggl_login": "google", "ptc_login": "ptc"}
...
class Device(Resource):
    ...
    def validate_custom(self) -> Optional[dict]:
        ...
        if self["ggl_login"] is not None:
            if self["ggl_login"] not in PogoAuth.get_avail_accounts(
                self._data_manager, auth_type="google", device_id=self.identifier
            ):
    ...
    def _load(self) -> None:
        ...
        for field, lookup_val in pogoauth_fields.items():
            search = {"device_id": self.identifier, "login_type": lookup_val}
