...
def parse_args():
    ...
    if "-cf" not in sys.argv and "--config" not in sys.argv:
        defaultconfigfiles = [os.getenv("MAD_CONFIG", os.path.join(mapadroid.MAD_ROOT, "configs/config.ini"))]
    if "-td" not in sys.argv and "--token_dispenser" not in sys.argv:
        default_tokenfile = os.getenv("MAD_CONFIG", os.path.join(mapadroid.MAD_ROOT, "configs/token-dispensers.ini"),)
    parser = configargparse.ArgParser(default_config_files=defaultconfigfiles, auto_env_var_prefix="THERAIDMAPPER_")
    ...
    # TODO - Depercate this
    parser.add_argument(
        "-dbm", "--db_method", required=False, default="rm", help="LEGACY: DB scheme to be used",
    )
    parser.add_argument("-dbip", "--dbip", required=False, help="IP or hostname of MySql Server")
    parser.add_argument("-dbport", "--dbport", type=int, default=3306, help="Port for MySQL Server")
    parser.add_argument("-dbuser", "--dbusername", required=False, help="Username for MySQL login")
    parser.add_argument("-dbpassw", "--dbpassword", required=False, help="Password for MySQL login")
    parser.add_argument("-dbname", "--dbname", required=False, help="Name of MySQL Database")
    parser.add_argument(
        "-dbps", "--db_poolsize", type=int, default=2, help="Size of MySQL pool (open connections to DB). Default: 2",
    )
    ...
    parser.add_argument(
        "-mspass", "--mitm_status_password", default="", help="Header Authorization password for MITM /status/ page",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="Kind of Job Messages to send - separated by pipe | " "(Default: SUCCESS|FAILURE|NOCONNECT|TERMINATED)",
    )
    parser.add_argument(
        "-jobrtnc",
    ...
    # Runtypes
    parser.add_argument(
        "-os", "--only_scan", action="store_true", default=True, help="Use this instance only for scanning",
    )
    parser.add_argument(
        "-otc",
        "--ocr_thread_count",
    ...
    parser.add_argument(
        "-cm", "--config_mode", action="store_true", default=False, help="Run in ConfigMode",
    )
    parser.add_argument(
        "-sn",
        "--status-name",
    ...
    parser.add_argument(
        "-nec", "--no_event_checker", action="store_true", help="Disable event checker task",
    )
    parser.add_argument(
        "-ut", "--unit_tests", action="store_true", default=False, help="Run unit tests then quit", dest="unit_tests",
    )

    # Path Settings
    parser.add_argument(
        "-tmp", "--temp_path", default="temp", help="Temp Folder for OCR Scanning. Default: temp",
    )
    parser.add_argument(
        "-upload",
        "--upload_path",
    ...
    parser.add_argument(
        "-L", "--language", default="en", help=("Set Language for MadMin / Quests. Default: en"),
    )

    # MADmin
    parser.add_argument(
        "-dm", "--disable_madmin", action="store_true", default=False, help="Disable Madmin on the instance",
    )
    parser.add_argument("-mmbp", "--madmin_base_path", default="/", help="Base path for madmin")
    parser.add_argument(
        "-mmip", "--madmin_ip", default="0.0.0.0", help="MADmin listening interface (Default: 0.0.0.0)",
    )
    parser.add_argument(
        "-mmprt", "--madmin_port", default="5000", help="MADmin web port (Default: 5000)",
    )
    parser.add_argument("-mmuser", "--madmin_user", default="", help="Username for MADmin Frontend.")
    parser.add_argument(
        "-mmpassword", "--madmin_password", default="", help="Password for MADmin Frontend.",
    )
    parser.add_argument(
        "-mmsc", "--madmin_sort", default="6", help="MADmin sort column Raid/Gym (5= Modify / 6 = Create) (Default: 6)",
    )
    parser.add_argument(
        "-mmt", "--madmin_time", default="24", help="MADmin clock format (12/24) (Default: 24)",
    )
    parser.add_argument(
        "-mmnrsp",
        "--madmin_noresponsive",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enables MADmin /quests_pub, /get_quests, and pushassets endpoints for public quests" "overview",
    )
    parser.add_argument(
        "-ods",
    ...
    # Statistics
    parser.add_argument(
        "-stat", "--statistic", action="store_true", default=False, help="Activate system statistics (Default: False)",
    )
    parser.add_argument(
        "-stco",
        "--stat_gc",
    ...
    # Game Stats
    parser.add_argument(
        "-ggs", "--game_stats", action="store_true", default=False, help="Generate worker stats",
    )
    parser.add_argument(
        "-ggrs",
        "--game_stats_raw",
    ...
    # ADB
    parser.add_argument(
        "-adb", "--use_adb", action="store_true", default=False, help='Use ADB for "device control" (Default: False)',
    )
    parser.add_argument(
        "-adbservip", "--adb_server_ip", default="127.0.0.1", help="IP address of ADB server (Default: 127.0.0.1)",
    )
    parser.add_argument(
        "-adpservprt", "--adb_server_port", type=int, default=5037, help="Port of ADB server (Default: 5037)",
    )

    # Webhook
    parser.add_argument(
        "-wh", "--webhook", action="store_true", default=False, help="Activate webhook support",
    )
    parser.add_argument(
        "-whurl",
        "--webhook_url",
    ...
    parser.add_argument("--log_path", default="logs/", help="Defines directory to save log files to.")
    parser.add_argument(
        "--log_filename",
        default="%Y%m%d_%H%M_<SN>.log",
        help=(
            "Defines the log filename to be saved."
    ...
    parser.add_argument(
        "--log_level",
        help=(
            "Forces a certain log level. By default"
    ...
    verbose.add_argument(
        ...
        default=0,
        dest="verbose",
        help=("Show debug messages. Has no effect, if" "--log_level has been set."),
    )
    parser.add_argument("--log_file_level", help="File logging level. See description for --log_level.")
    parser.add_argument(
        "--log_file_retention",
        default="10",
        help=("Amount of days to keep file logs. Set to 0 to" " keep them forever (Default: 10)"),
    )
    parser.add_argument(
        "--no_log_colors", action="store_true", default=False, help=("Disable colored logs."),
    )
    parser.set_defaults(DEBUG=False)

    # MADAPKs
    parser.add_argument(
        "-td", "--token_dispenser", default=default_tokenfile, help="Token dispenser config (MAD)",
    )
    parser.add_argument(
        "-tdu", "--token_dispenser_user", default="", help="Token dispenser config (User)",
    )
    parser.add_argument(
        "-gu", "--gmail_user", default="", help="Google Mail User for interacting with the Google Play Store",
    )
    parser.add_argument(
        "-gp",
        "--gmail_passwd",
