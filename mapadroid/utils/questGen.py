...
def questreward(quest_reward_type):
    type = {2: _("Item"), 3: _("Stardust"), 7: _("Pokemon"), 12: _("Energy")}
    ...
    if typeid == 4:
        ...
        if match_object is not None:
            ...
            if last == 1:
                ...
                for ty in pt:
                    arr["poke"] += (
                        (_("or ") if last == cur else "") + i8ln(pokemonname(ty)) + ("" if last == cur else ", ")
                    )
                    cur += 1
            text = _("Catch {0} {poke}")
    elif typeid == 5:
        ...
        for con in condition_dict:
            if con.get("type", 0) == 22:
                ...
                in_go_battle_league = any(
                    x in con.get("with_pvp_combat", {}).get("combat_league_template_id", [])
                    for x in [
