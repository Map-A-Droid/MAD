...
class DeviceUpdater(object):
    def __init__(self, websocket, args, returning, db, storage_obj: AbstractAPKStorage):
        ...
        try:
            ...
            logger.error(
                "Corrupted update_log.json file found. Deleting the file. Please check remaining disk space "
                "or disk health."
            )
    ...
    def restart_job(self, job_id: int):
        if (job_id) in self._log:
            ...
            if redo:
                algo = (
                    self.get_job_algo_value(
                        algotyp=self._globaljoblog[globalid].get("algotype", "flex"),
                        algovalue=self._globaljoblog[globalid].get("algovalue", 0),
                    )
                    + waittime
                )
                ...
                self.add_job(
                    globalid, origin, file_, job_id=job_id, job_type=jobtype, status="requeued", jobname=jobname
                )
    ...
    def process_update_queue(self, threadnumber):
        ...
        while not self._stop_updater_threads.is_set():
            try:
                ...
                if (
                    laststatus is not None
                    and laststatus == "faulty"
                    and self._globaljoblog[globalid].get("autojob", False)
                ):
                ...
                if (
                    (laststatus is None or laststatus == "future")
                    and not startwithinit
                ...
                if (
                    laststatus is not None
                    and laststatus in ("pending", "future", "failure", "interrupted", "not connected")
                    and lastjobid != job_id
                    and processtime is None
                ):
                    logger.debug(
                ...
                if job_id in self._log:
                        ...
                        if temp_comm is None or temp_comm is False:
                            ...
                            try:
                                if self.start_job_type(item, jobtype, temp_comm):
                                    ...
                                    logger.error(
                                        "Job {} could not be executed successfully - Device {} - File/Job {} "
                                        "(ID: {})",
                                        jobtype,
                                        origin,
                                        file_,
                                ...
                                logger.error(
                                    "Job {} could not be executed successfully (fatal error) - Device {} - "
                                    "File/Job {} (ID: {})",
                                    jobtype,
                                    origin,
                                    file_,
                    ...
                    if jobstatus not in SUCCESS_STATES and (
                        ...
                        if redo and self._globaljoblog[globalid].get("redoonerror", False):
                            logger.info(
                                "Re-add this automatic job for {} (File/Job: {} - Type {})  (ID: {})",
                                origin,
                                file_,
                                jobtype,
                                job_id,
            ...
            if jobtype == JobType.INSTALLATION:
                ...
                try:
                    ...
                    if package_ver_job and package_ver_job.split("\n")[0].strip() == "OK":
                        ...
                        logger.warning(
                            "Unable to determine version for {}: {}", self._log[str(item)]["file"], package_ver_job
                        )
                        return False
                ...
                if not is_newer_version(mad_apk.version, package_ver):
                    ...
                    logger.info(
                        "Smart Update APK Installation for {} to {}", package.name, self._log[str(item)]["origin"]
                    )
                    apk_file = bytes()
                    ...
                    return (
                        returning
                        if not "RemoteGpsController".lower() in str(self._log[str(item)]["file"]).lower()
                        else True
                    )
    ...
    def send_webhook(self, job_id, status):
        ...
        try:
            ...
            embed.add_embed_field(
                name="Next run", value=str(datetime.fromtimestamp(processtime) if processtime is not None else "-")
            )
            ...
            for autocommand in autocommands:
                ...
                for origin in origins:
                    ...
                    algo = self.get_job_algo_value(
                        algotyp=autocommand.get("algotype", "flex"), algovalue=autocommand.get("algovalue", 0)
                    )
    ...
    def get_job_algo_value(self, algotyp, algovalue):
        ...
        tm = datetime.now().replace(hour=int(algotime[0]), minute=int(algotime[1]), second=0, microsecond=0)
