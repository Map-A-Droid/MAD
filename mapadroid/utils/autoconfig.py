    ...
    def get_issues(self) -> Tuple[List[str], List[str]]:
        ...
        if AutoConfIssues.no_ggl_login in self.warnings:
            ...
            issues_warning.append(
                "No available Google logins for auto creation of devices. Configure through " f"{anchor}"
            )
...
class AutoConfigCreator:
    ...
    def delete(self):
        del_info = {"name": self.source, "instance_id": self._db.instance_id}
...
class PDConfig(AutoConfigCreator):
    ...
    def load_config(self) -> NoReturn:
        ...
        if self.contents["post_destination"] in ["http://", ""]:
            self.contents["post_destination"] = "http://{}:{}".format(
                self._args.mitmreceiver_ip, self._args.mitmreceiver_port
            )
        auths = self._data_manager.get_root_resource("auth")
        if auths:
