...
class DataManager(object):
    ...
    def generate_uri(self, location, *args):
        try:
            ...
            location_args = [mapadroid.madmin.api.BASE_URI, self.translate_location(location), *args]
    ...
    def get_api_attribute(self, location, attribute):
        return getattr(mapadroid.madmin.api.valid_modules[location], attribute)
    ...
    def get_data(self, location, identifier=None, **kwargs):
        ...
        if identifier is None and kwargs.get('uri', True):
            ...
            try:
                ...
                data = self.get_sorted_data(data, self.get_api_attribute(location, 'default_sort'), location,
                                            fetch_all)
        return data
    ...
    def set_data(self, data, location, action, **kwargs):
        ...
        if action == 'patch':
            if identifier not in self.__raw[config_section]['entries']:
                raise KeyError
            self.__raw[config_section]['entries'][identifier] = self.recursive_update(
                self.__raw[config_section]['entries'][identifier],
                data,
                append=append)
            self.save_config()
            return True
