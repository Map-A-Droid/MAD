...
class WorkerQuests(MITMBase):
        ...
        if delay_used < 0:
            ...
            if lastcleanupbox is not None:
                if time.time() - lastcleanupbox > 900:
                    ...
                    cleanupbox = True
            self._mapping_manager.routemanager_set_worker_sleeping(self._routemanager_name, self._id,
                                                                   delay_used)
    ...
    def _update_injection_settings(self):
        ...
        self._mitm_mapper.update_latest(origin=self._id, key="ids_encountered",
                                        values_dict=self._encounter_ids)
        self._mitm_mapper.update_latest(origin=self._id, key="ids_iv", values_dict=ids_iv)

        self._mitm_mapper.update_latest(origin=self._id, key="injected_settings",
    ...
    def _open_pokestop(self, timestamp: float):
        ...
        if not self._current_position_has_spinnable_stop(timestamp):
            ...
            if data_received != LatestReceivedType.UNDEFINED and not self._current_position_has_spinnable_stop(
                    timestamp):
                logger.info(
