...
class MITMBase(WorkerBase):
    ...
    def _walk_after_teleport(self, walk_distance_post_teleport) -> float:
        ...
        self._communicator.walk_from_to(
            self.current_location,
            Location(self.current_location.lat + lat_offset, self.current_location.lng + lng_offset,),
    ...
    def _wait_for_data(
        self, timestamp: float = None, proto_to_wait_for: ProtoIdentifier = ProtoIdentifier.GMO, timeout=None,
        ...
        self.logger.debug(
            "Waiting for data ({}) after {} with timeout of {}s. " "Last received timestamp of that type was: {}",
            proto_to_wait_for,
        ...
        while (
            type_of_data_returned == LatestReceivedType.UNDEFINED
            and (int(timestamp + timeout) >= int(time.time()) or last_time_received >= timestamp)
            ...
            if (
                ...
                or latest_location.lat == latest_location.lng == 1000
                or not (
                    latest_location.lat != 0.0
                    and latest_location.lng != 0.0
                    and -90.0 <= latest_location.lat <= 90.0
                    and -180.0 <= latest_location.lng <= 180.0
                )
            ):
                self.logger.debug(
                    "Data may be valid but does not contain a proper location yet: {}", str(latest_location),
                )
                check_data = False
            ...
            if check_data:
                type_of_data_returned, data = self._check_for_data_content(latest, proto_to_wait_for, timestamp)
    ...
    def _handle_proto_timeout(self, position_type, proto_to_wait_for: ProtoIdentifier, type_of_data_returned):
        self.logger.info(
            "Timeout waiting for useful data. Type requested was {}, received {}",
            proto_to_wait_for,
        ...
        if self._restart_count > restart_thresh:
            ...
            if self._reboot_count > reboot_thresh and self.get_devicesettings_value("reboot", True):
    ...
    def raise_stop_worker_if_applicable(self):
        ...
        """
        if not self._mapping_manager.routemanager_present(self._routemanager_name) or self._stop_worker_event.is_set():
    ...
    def _is_location_within_allowed_range(self, latest_location):
        ...
        self.logger.debug2(
            "Distance of worker {} to (data) location: {}", self._origin, distance_to_data,
        )
        if distance_to_data > max_distance_for_worker:
            self.logger.debug(
                "Location too far from worker position, max distance allowed: {}m", max_distance_for_worker,
            )
    ...
    def _wait_for_injection(self):
        ...
        while not self._mitm_mapper.get_injection_status(self._origin):
            ...
            if reboot and self._not_injected_count >= injection_thresh_reboot:
                ...
                return False
            self.logger.info(
                "Didn't receive any data yet. (Retry count: {}/{})", self._not_injected_count, injection_thresh_reboot,
            )
            if (
                self._not_injected_count != 0 and self._not_injected_count % window_check_frequency == 0
            ) and not self._stop_worker_event.is_set():
                self.logger.info(
                    "Retry check_windows while waiting for injection at count {}", self._not_injected_count,
                )
    ...
    def _check_for_data_content(
        self, latest, proto_to_wait_for: ProtoIdentifier, timestamp
    ...
    def _walk_to_location(self, speed: float) -> int:
        ...
        if delay_used - SECONDS_BEFORE_ARRIVAL_OF_WALK_BUFFER < time_before_walk:
            ...
            math.floor((math.floor(time.time()) + time_before_walk) / 2)
    ...
    def worker_stats(self):
        ...
        if routemanager_status is None:
            ...
            self.logger.debug(
                "Route Pos: {} - Route Length: {}", routemanager_status[0], routemanager_status[1],
            )
