...
from mapadroid.utils.logging import LoggerEnums, get_logger, routelogger_set_origin
...
class RouteManagerBase(ABC):
    def __init__(
        ...
        path_to_include_geofence: GeoFence,
        ...
        self.geofence_helper: GeofenceHelper = GeofenceHelper(path_to_include_geofence, path_to_exclude_geofence)
        ...
        if coords is not None:
            ...
            new_coords = self._route_resource.get_json_route(
                fenced_coords,
                int(max_radius),
        ...
        if self.settings is not None:
            ...
            if mode == "mon_mitm":
                self.remove_from_queue_backlog = self.settings.get("remove_from_queue_backlog", 300)
            else:
                self.remove_from_queue_backlog = self.settings.get("remove_from_queue_backlog", 0)
        else:
    ...
    def _start_check_routepools(self):
        self._check_routepools_thread = Thread(name=self.name + " - _check_routepools", target=self._check_routepools)
    ...
    def unregister_worker(self, worker_name):
        ...
        with self._workers_registered_mutex and self._manager_mutex:
            ...
            if len(self._workers_registered) == 0 and self._is_started:
                self.logger.info(
                    "Routemanager does not have any subscribing workers anymore, calling stop", self.name,
                )
        ...
        if (self.delay_after_timestamp_prio is not None or self.mode == "iv_mitm") and not self.mode == "pokestops":
            ...
            if self.mode not in ["iv_mitm", "pokestops"]:
                if self.mode == "mon_mitm" and self.settings is not None:
                    max_clustering = self.settings.get("max_clustering", self._max_coords_within_radius)
                    if max_clustering == 0:
                        max_clustering = self._max_coords_within_radius
                else:
                    max_clustering = self._max_coords_within_radius
                self.clustering_helper = ClusteringHelper(
                    self._max_radius, max_clustering, self._cluster_priority_queue_criteria(),
                )
            self._update_prio_queue_thread = Thread(
                name=self.name + "- prio_queue_update", target=self._update_priority_queue_loop,
            )
    ...
    def calculate_new_route(
        ...
        if len(coords) > 0:
            new_route = self._route_resource.calculate_new_route(
                coords,
                max_radius,
                max_coords_within_radius,
                delete_old_route,
    ...
    def initial_calculation(
        self, max_radius: float, max_coords_within_radius: int, num_procs: int = 1, delete_old_route: bool = False,
    ):
        if not self._route_resource["routefile"]:
            ...
            self.recalc_route(
                max_radius, max_coords_within_radius, num_procs=0, delete_old_route=False,
            )
    ...
    def recalc_route_adhoc(
        ...
        new_route = self.recalc_route(
            max_radius, max_coords_within_radius, num_procs, in_memory=True, calctype=calctype,
        )
        calc_coords = []
        for coord in new_route:
        ...
        if new_queue is not None:
            with self._manager_mutex:
                ...
                if self.mode == "mon_mitm":
                    ...
                    self.logger.debug2(
                        "Merging existing Q of {} events with {} clustered new events",
                        len(self._prio_queue),
                        len(new_queue),
                    )
    ...
    def _get_round_finished_string(self):
        ...
        round_completed_in = "%d hours, %d minutes, %d seconds" % (
            self.dhms_from_seconds(self.date_diff_in_seconds(round_finish_time, self._round_started_time))
        )
    ...
    def _filter_priority_queue_internal(self, latest, cluster=True):
        ...
        if self.mode == "mon_mitm" and self.remove_from_queue_backlog == 0:
            self.logger.warning(
                "You are running in mon_mitm mode with priority queue enabled and "
        ...
        with self._manager_mutex:
            ...
            if self.delay_after_timestamp_prio is not None and (
                ...
                if self._other_worker_closer_to_prioq(next_coord, origin):
                    ...
                    route_logger.info(
                        "Prio event scheduled for {} passed to a closer worker.", next_readable_time,
                    )
                ...
                self._positiontyp[origin] = 1
                route_logger.info(
                    "Moving to {}, {} for a priority event scheduled for {}",
                    next_coord.lat,
                    next_coord.lng,
                    next_readable_time,
                )
            ...
            if self._check_worker_rounds() > self._roundcount:
                ...
                if self._round_started_time is not None:
                    self.logger.info(
                        "All subroutes reached the first spot again. It took {}", self._get_round_finished_string(),
                    )
            ...
            if (
                self.init
                and self._check_worker_rounds() >= int(self.settings.get("init_mode_rounds", 1))
                and len(self._routepool[origin].queue) == 0
            ):
                # we are done with init, let's calculate a new route
                self.logger.warning(
                    "Init done, it took {}, calculating new route...", self._get_round_finished_string(),
                )
                if self._start_calc:
                ...
                if len(self._routepool[origin].queue) == 0 and len(self._routepool[origin].subroute) == 0:
                    route_logger.info(
                        "Subroute-update won't help or queue and subroute are empty, signaling worker to " "reconnect"
                    )
                    self._routepool[origin].last_access = time.time()
                    return None
            ...
            if len(self._routepool[origin].queue) == 0:
                route_logger.warning(
                    "Having updated routepools and checked lengths of queue and subroute, "
            ...
            while len(self._routepool[origin].queue) > 0:
                ...
                if (
                    self._delete_coord_after_fetch()
                    and next_coord in self._current_route_round_coords
                    and not self.init
                ):
                    self._current_route_round_coords.remove(next_coord)
                route_logger.info(
                    "Moving on with location {}, {} [{} coords left (Workerpool)]",
                    next_coord.lat,
                    next_coord.lng,
                    len(self._routepool[origin].queue) + 1,
                )
    ...
    def _check_coord_and_maybe_del(self, next_coord, origin):
        route_logger = routelogger_set_origin(self.logger, origin=origin)
        route_logger.debug(
            "Done grabbing next coord, releasing lock and returning location: {}", next_coord,
        )
        if self._check_coords_before_returning(next_coord.lat, next_coord.lng, origin):
            if self._delete_coord_after_fetch() and next_coord in self._current_route_round_coords and not self.init:
                self._current_route_round_coords.remove(next_coord)
    ...
    def _other_worker_closer_to_prioq(self, prioqcoord, origin):
        ...
        distance_worker = get_distance_of_two_points_in_meters(
            current_worker_pos.lat, current_worker_pos.lng, prioqcoord.lat, prioqcoord.lng,
        )
        ...
        for worker in self._routepool.keys():
            if (
                worker == origin
                or self._routepool[worker].has_prio_event
            ...
            if prio_distance < temp_distance:
                route_logger.debug(
                    "Worker {} closer by {} meters", worker, int(distance_worker) - int(prio_distance),
                )
    ...
    def _worker_changed_update_routepools(self):
        ...
        with self._manager_mutex and self._workers_registered_mutex:
            ...
            self.logger.info(
                "Current route for all workers length: {}", len(self._current_route_round_coords),
            )
            ...
            if workers > len(self._current_route_round_coords):
                ...
                try:
                    new_subroute_length = math.floor(len(self._current_route_round_coords) / workers)
            ...
            if extra_length_workers > 0:
                self.logger.debug(
                    "New subroute length: {}-{}", new_subroute_length, new_subroute_length + 1,
                )
            else:
            ...
            with self._workers_registered_mutex:
                ...
                for origin, _time_added in sorted_routepools:
                    if origin not in self._routepool:
                        ...
                        self.logger.info(
                            "{} must have unregistered when we weren't looking.. skip it", origin,
                        )
                        continue
                    entry: RoutePoolEntry = self._routepool[origin]
                    ...
                    if len(entry.subroute) == 0:
                        self.logger.debug(
                            "{}'s subroute is empty, assuming he has freshly registered and desperately "
                            "needs a queue",
                            origin,
                        )
                        ...
                        if compare(new_subroute, entry.subroute):
                            ...
                            self.logger.debug(
                                "new_subroute == entry.subroute: {}", new_subroute == entry.subroute,
                            )
                            entry.subroute = new_subroute
                            entry.queue.clear()
                        ...
                        self.logger.debug(
                            "{}'s subroute is longer than it should be now (maybe a worker has been " "added)", origin,
                        )
                        ...
                        if len(old_queue) == 0:
                            ...
                            if last_el_old_q in new_subroute:
                                ...
                                self.logger.debug(
                                    "Last element of old queue is present in new subroute, appending the "
                                ...
                                while len(new_subroute_copy) > 0 and new_subroute_copy.popleft() != last_el_old_q:
                                    pass
                                self.logger.debug(
                                    "Length of subroute to be extended by {}", len(new_subroute_copy),
                                )
                                ...
                                self.logger.debug(
                                    "Subroute of {} has changed. Replacing entirely", origin,
                                )
                                entry.queue.clear()
                                new_subroute_copy = collections.deque(new_subroute)
                                while len(new_subroute_copy) > 0:
                        ...
                        if last_el_new_route is not None and last_el_new_route in old_queue_list:
                            ...
                            self.logger.debug(
                                "Last element of new subroute is in old queue, removing everything after "
                                "that element"
                            )
                            ...
                            self.logger.debug(
                                "Worker {} getting a completely new route - replace it", origin,
                            )
                            new_subroute_copy = collections.deque(new_subroute)
