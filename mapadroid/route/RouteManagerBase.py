...
from mapadroid.route.routecalc.calculate_route import getJsonRoute
...
class RouteManagerBase(ABC):
    def __init__(self, db_wrapper: DbWrapperBase, dbm: DataManager, uri: str, coords: List[Location],
                 max_radius: float,
    ...
    def get_next_location(self, origin: str) -> Optional[Location]:
        ...
        with self._manager_mutex:
            ...
            if (self.delay_after_timestamp_prio is not None and ((not self._last_round_prio.get(origin, False)
                ...
                # continue as usual
                if self.init and self.check_worker_rounds() >= int(
                        self.settings.get("init_mode_rounds", 1)) and \
                    ...
                    if len(self._routepool[origin].queue) == 0 and len(self._routepool[origin].subroute) == 0:
                        ...
                        self._routepool[origin].last_access = time.time()
                        return None
                    elif len(self._routepool[origin].queue) == 0 and len(
                            self._routepool[origin].subroute) > 0:
