...
from flask import Response, jsonify, redirect, render_template, send_file, url_for

from mapadroid.madmin.functions import auth_required
from mapadroid.utils.autoconfig import AutoConfIssueGenerator, PDConfig, RGCConfig
...
class AutoConfigManager(object):
    ...
    def autoconfig_download_file(self):
        ...
        return send_file(
            config_file, as_attachment=True, attachment_filename="mad_autoconf.txt", mimetype="text/plain",
        )
    ...
    def autoconf_logs_get(self, session_id):
        ...
        sql = (
            "SELECT UNIX_TIMESTAMP(`log_time`) as 'log_time', `level`, `msg`\n"
    ...
    def autoconfig_pending(self):
        ...
        pending = {}
        sql = (
            "SELECT ar.`session_id`, ar.`ip`, sd.`device_id`, sd.`name` AS 'origin', ar.`status`"
    ...
    def autoconfig_pending_dev(self, session_id):
        ...
        if not session:
            return redirect(url_for("autoconfig_pending"), code=302)
        sql = (
            "SELECT ag.`account_id`, ag.`username`\n"
            "FROM `settings_pogoauth` ag\n"
