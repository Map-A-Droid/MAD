...
class MADminStatistics(object):
    ...
    def statistics_mon(self):
        ...
        return render_template(
            "statistics/mon_statistics.html",
    ...
    def statistics_shiny(self):
        return render_template(
            "statistics/shiny_statistics.html",
    ...
    def statistics_stop_quest(self):
        return render_template(
            "statistics/stop_quest_statistics.html",
            ...
            location_info.append(
                {
                    "worker": str(dat[0]),
                    "locations": str(dat[1]),
                    "locationsok": str(dat[2]),
                    "locationsnok": str(dat[3]),
                    "ratio": str(dat[4]),
                }
            )
        ...
        if data is not None:
            for dat in data:
                ...
                good_spawns.append(
                    {
                        "id": dat[1],
                        "iv": round(calculate_iv(dat[3], dat[4], dat[5]), 0),
                        "lvl": lvl,
                        "cp": dat[7],
                        "img": mon_img,
                        "name": mon_name,
                        "periode": datetime.datetime.fromtimestamp(self.utc2local(dat[2])).strftime(
            ...
            shiny_stats_v2.append(
                {
                    ...
                    "worker": dat[8],
                    "lat": dat[2],
                    "lat_5": "{:.5f}".format(dat[2]),
                    "lng_5": "{:.5f}".format(dat[3]),
                    "lng": dat[3],
                    "timestamp": timestamp.strftime(self._datetimeformat),
                    "form": dat[1],
                    "mon_id": dat[0],
                    "encounter_id": str(dat[9]),
                }
            )
        ...
        for dat in data:
            if dat[1] in shiny_count and dat[2] in shiny_count[dat[1]]:
                ...
                global_shiny_stats_v2.append(
                    {
                        "name": mon_names[dat[1]],
                        "count": dat[0],
                        "img": mon_img,
                        "shiny": shiny_count[dat[1]][dat[2]],
                        "odds": odds,
                        "mon_id": dat[1],
                        "form": dat[2],
                        "gender": dat[3],
    ...
    def status(self):
        return render_template(
            "status.html", responsive=str(self._args.madmin_noresponsive).lower(), title="Worker status",
        )
            ...
            for subfence in possible_fences[possible_fence]["include"]:
                ...
                for event in events:
                    ...
                    coords.append(
                        {
                            "fence": subfence,
                            "known": len(known[event]),
                            "unknown": len(unknown[event]),
                            "sum": len(known[event]) + len(unknown[event]),
                            "event": event,
                            "mode": mode,
                            "area_id": area_id,
                            "eventid": eventidhelper[event],
    ...
    def delete_spawns(self):
        ...
        if area_id is not None and event_id is not None:
            self._db.delete_spawnpoints(
                self.get_spawnpoints_from_id(area_id, event_id, olderthanxdays=olderthanxdays, index=index)
            )
    ...
    def convert_spawns(self):
        ...
        if self._db.check_if_event_is_active(event_id):
            ...
            return jsonify({"status": "event"})
        if area_id is not None and event_id is not None:
            self._db.convert_spawnpoints(
                self.get_spawnpoints_from_id(area_id, event_id, todayonly=todayonly, index=index)
            )
        if todayonly:
    ...
    def get_spawnpoints_from_id(self, spawn_id, eventid, todayonly=False, olderthanxdays=None, index=0):
        ...
        data = json.loads(
            self._db.download_spawns(
                fence=str(fence), eventid=int(eventid), todayonly=bool(todayonly), olderthanxdays=olderthanxdays,
            )
        )
    ...
    def statistics_spawns(self):
        return render_template(
            "statistics/spawn_statistics.html",
    ...
    def get_spawn_details(self):
        ...
        return jsonify(
            self.get_spawn_details_helper(
                areaid=area_id, eventid=event_id, olderthanxdays=olderthanxdays, todayonly=todayonly, index=index,
            )
        )

    def get_spawn_details_helper(
        self, areaid, eventid, todayonly=False, olderthanxdays=None, sumonly=False, index=0,
    ):
        active_spawns: list = []
    ...
    def spawn_details(self):
        ...
        index = request.args.get("index", 0)
        return render_template(
            "statistics/spawn_details.html",
            title="MAD Spawnpoint Details",
            time=self._args.madmin_time,
            responsive=str(self._args.madmin_noresponsive).lower(),
            areaid=area_id,
            eventid=event_id,
