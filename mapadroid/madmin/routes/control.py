...
from mapadroid.utils.functions import creation_date, generate_path, generate_phones, image_resize
...
class MADminControl(object):
                ...
                for pho in devicemappings:
                    if phonename.serial == devicemappings[pho].get("adb", False):
                        ...
                        if os.path.isfile(filename):
                            image_resize(
                                filename, os.path.join(mapadroid.MAD_ROOT, self._args.temp_path, "madmin"), width=250,
                            )
                            screenshot_ending: str = ".jpg"
                            ...
                            screens_phone.append(
                                generate_phones(
                                    pho, add_text, adb_option, screen, filename, self._datetimeformat, dummy=True,
                                )
                            )

        return render_template(
            "phonescreens.html", editform=screens_phone, header="Device control", title="Device control",
        )
    ...
    def take_screenshot(self, origin=None, adb=False):
        ...
        creationdate = datetime.datetime.fromtimestamp(creation_date(filename)).strftime(self._datetimeformat)
    ...
    def generate_screenshot(self, origin):
        ...
        temp_comm.get_screenshot(
            generate_device_screenshot_path(origin, devicemappings, self._args), screenshot_quality, screenshot_type,
        )
    ...
    def swipe_screenshot(self):
        ...
        if useadb == "True" and self._adb_connect.make_screenswipe(
            adb, origin, real_click_x, real_click_y, real_click_xe, real_click_ye
        ):
            origin_logger.info("MADmin: ADB screenswipe successfully")
        else:
            origin_logger.info(
                "MADmin WS Swipe x:{} y:{} xe:{} ye:{}", real_click_x, real_click_y, real_click_xe, real_click_ye,
            )
            temp_comm = self._ws_server.get_origin_communicator(origin)
            temp_comm.touch_and_hold(
                int(real_click_x), int(real_click_y), int(real_click_xe), int(real_click_ye),
            )
    ...
    def get_logcat(self):
        ...
        if self._fetch_logcat_websocket(origin, filename):
            ...
            return send_file(
                generate_path(filename), as_attachment=True, attachment_filename="logcat_{}.zip".format(origin),
            )
        else:
    ...
    def clear_game_data(self):
        ...
        if useadb == "True" and self._adb_connect.send_shell_command(adb, origin, "pm clear com.nianticlabs.pokemongo"):
            origin_logger.info("MADmin: ADB shell command successfully")
        else:
            temp_comm = self._ws_server.get_origin_communicator(origin)
    ...
    def install_file(self):
        ...
        if os.path.exists(os.path.join(self._args.upload_path, jobname)):
            if useadb == "True":
                if self._adb_connect.push_file(
                    adb, origin, os.path.join(self._args.upload_path, jobname)
                ) and self._adb_connect.send_shell_command(
