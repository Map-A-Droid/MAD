...
class ResourceHandler(apiHandler.APIHandler):
    ...
    def get_resource_data_root(self, resource_def, resource_info):
        ...
        if hide_resource:
            ...
            response_data = {"resource": resource_info, "results": api_response_data}
    ...
    def process_request(self, *args, **kwargs):
        ...
        try:
            ...
            error = {"error": msg % (err.mode, ",".join(self._data_manager.get_valid_modes(self.component)),)}
            return (error, 400)
        except (ModeNotSpecified, NoModeSpecified):
            msg = "Please specify a mode for resource information.  Valid modes: %s"
            error = {"error": msg % (",".join(self._data_manager.get_valid_modes(self.component)),)}
            return (error, 400)
        ...
        try:
            ...
            for section, identifier in err.dependencies:
                ...
                try:
                    ...
                    errors.append({"name": name, "uri": "%s/%s" % (flask.url_for("api_%s" % (section,)), identifier)})
                except InvalidSection:
                    errors.append({"name": section, "identifier": identifier})
            return (errors, 412)
        else:
            headers = {"X-Status": "Successfully deleted the object"}
    ...
    def put(self, identifier, data, resource_def, resource_info, *args, **kwargs):
        ...
        try:
            ...
            headers = {"X-Status": "Object does not exist to update"}
            return (None, 404, {"headers": headers})
        else:
            headers = {"X-Status": "Successfully replaced the object"}
            return (None, 204, {"headers": headers})
