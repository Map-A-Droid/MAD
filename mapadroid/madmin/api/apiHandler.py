...
class ResourceHandler(object):
    ...
    def create_routes(self):
        ...
        if self.component:
            ...
            if self.iterable:
                ...
                self._app.route(route, methods=['DELETE', 'GET', 'PATCH', 'PUT'], endpoint=self.component)(
    ...
    def format_data(self, data, config, operation):
        ...
        for section in sections:
            if section == 'fields':
                ...
                try:
                    (tmp_save, tmp_inv, tmp_missing) = self.format_section(data[section], config[section],
                                                                           operation)
    ...
    def get_required_configuration(self, mode=None):
        ...
        if type(self.configuration) is dict and (
                'fields' in self.configuration or 'settings' in self.configuration):
    ...
    def process_request(self, endpoint=None, identifier=None):
        ...
        if identifier is None and flask.request.method != 'POST':
            ...
            raw_data = self._data_manager.get_data(self.component, fetch_all=fetch_all,
                                                   display_field=disp_field, mode=mode)
            if hide_resource:
            ...
            (self.api_req.data, invalid, missing) = self.format_data(self.api_req.data, config,
    ...
    def delete(self, identifier, *args, **kwargs):
        ...
        except mapadroid.utils.data_manager.DataManagerDependencyError as err:
    ...
    def get(self, identifier, *args, **kwargs):
        ...
        try:
            return apiResponse.APIResponse(self._logger, self.api_req)(
                self._data_manager.get_data(self.component, identifier=identifier), 200)
        except KeyError:
            return apiResponse.APIResponse(self._logger, self.api_req)(None, 404)
    ...
    def patch(self, identifier, *args, **kwargs):
        ...
        try:
            self._data_manager.set_data(self.api_req.data, self.component, 'patch', identifier=identifier,
                                        append=append)
        except KeyError:
            return apiResponse.APIResponse(self._logger, self.api_req)(None, 404)
        else:
    ...
    def post(self, identifier, *args, **kwargs):
        ...
        except mapadroid.utils.data_manager.DataManagerInvalidMode as err:
            return apiResponse.APIResponse(self._logger, self.api_req)(
                'Invalid mode specified: %s' % (err.mode,), 400)
        headers = {
            'Location': uri_key,
            'X-Uri': uri_key,
