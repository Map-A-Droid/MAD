...
class GetStopsEndpoint(AbstractRootEndpoint):
    ...
    async def get(self):
        ...
        for stop in data:
            stop_serialized = vars(stop)
            stop_serialized["last_modified"] = int(stop.last_modified.replace(tzinfo=timezone.utc).timestamp())
            stop_serialized["lure_expiration"] = int(stop.lure_expiration.replace(tzinfo=timezone.utc).timestamp())
            stop_serialized["last_updated"] = int(stop.last_updated.replace(tzinfo=timezone.utc).timestamp())
            stop_serialized["incident_start"] = int(stop.incident_start.replace(tzinfo=timezone.utc).timestamp())
            stop_serialized["incident_expiration"] = int(
                stop.incident_expiration.replace(tzinfo=timezone.utc).timestamp())
